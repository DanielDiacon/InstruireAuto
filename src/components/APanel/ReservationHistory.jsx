          ...
          {formattedReservations.map((entry, index) => (
            ...
            >
              <div className="history__item-left">
                <h3>{entry.person}</h3>
                <p>{entry.instructor ? `cu ${entry.instructor}` : "fără instructor"}</p>
                <span>{entry.time}</span>
              </div>
              <div className="history__item-right">
                {entry.status === "completed" && <ReactSVG className="history__item-icon completed" src={successIcon} />}
                {entry.status === "cancelled" && <ReactSVG className="history__item-icon cancelled" src={cancelIcon} />}
                {entry.status === "pending" && <ReactSVG className="history__item-icon pending" src={clockIcon} />}
              </div>
            </div>
          ))}
        </div>
