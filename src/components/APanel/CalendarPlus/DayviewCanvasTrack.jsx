...
function reservationIdFromAny(node) {
   return (
      ...
      node?.reservationId ??
      node?.reservation?.id ??
      node?.reservation?.reservationId ??
      null
   );
}
...
function reservationColorFromAny(node) {
   return node?.color ?? node?.reservation?.color ?? null;
}

function reservationSectorFromAny(node) {
   return node?.sector ?? node?.reservation?.sector ?? null;
}

function reservationGearboxFromAny(node) {
   return node?.gearbox ?? node?.reservation?.gearbox ?? null;
}

function reservationPrivateMessageFromAny(node) {
   return (
      node?.privateMessage ??
      node?.privateMessaje ??
      node?.reservation?.privateMessage ??
      node?.reservation?.privateMessaje ??
      ""
   );
}
...
function extractReservationLikeRows(payload) {
   const out = [];
   const seen = new Set();
...
function pickCreatedReservationFromPasteResult(
   createResult,
   {
      startTimeToSend,
      userIdNum,
   ...
   const rows = extractReservationLikeRows(createResult);
   ...
   for (const row of rows) {
      const idRaw = reservationIdFromAny(row);
      const startRaw = reservationStartFromAny(row);
      const userIdRaw = reservationUserIdFromAny(row);
      const instructorIdRaw = reservationInstructorIdFromAny(row);

      const userId = Number(userIdRaw);
      const instructorId = Number(instructorIdRaw);
      const slotKey = toSlotKeyForCompare(startRaw);

      let score = 0;
      if (idRaw != null) score += 2;
      if (Number.isFinite(userId) && userId === userIdNum) score += 4;
      if (Number.isFinite(instructorId) && instructorId === instructorIdNum)
         score += 4;
   ...
   const idRaw = reservationIdFromAny(best);
   if (idRaw == null) return null;

   const idNum = Number(idRaw);
   const normalizedId = Number.isFinite(idNum) ? idNum : String(idRaw);

   const startRaw = reservationStartFromAny(best) || startTimeToSend;
   const userRaw = reservationUserIdFromAny(best);
   const instructorRaw = reservationInstructorIdFromAny(best);

   const userIdOut = Number(userRaw);
   const instructorIdOut = Number(instructorRaw);

   const colorRaw = reservationColorFromAny(best);
   const colorOut =
      typeof colorRaw === "string" && colorRaw.trim()
         ? colorRaw.trim()
         : fallbackColor;

   const sectorRaw = reservationSectorFromAny(best);
   const sectorOut =
      typeof sectorRaw === "string" && sectorRaw.trim()
         ? sectorRaw.trim()
         : fallbackSector;

   const gearboxRaw = reservationGearboxFromAny(best);
   const gearboxOut = normalizeGearboxValue(gearboxRaw || fallbackGearbox);

   const privateMessageRaw = reservationPrivateMessageFromAny(best);

   return {
      id: normalizedId,
      userId: Number.isFinite(userIdOut) ? userIdOut : userIdNum,
      instructorId: Number.isFinite(instructorIdOut)
         ? instructorIdOut
         : instructorIdNum,
      startTime: String(startRaw || startTimeToSend),
      sector: sectorOut,
      gearbox: gearboxOut,
      privateMessage:
         typeof privateMessageRaw === "string"
            ? privateMessageRaw
            : fallbackPrivateMessage,
      color: colorOut,
      _optimistic: false,
...
function DayviewCanvasTrack({
   ...
   useEffect(() => {
      ...
      const overscanX = isPanInteracting
         ? Math.max(82, Math.round(colWidth * 0.8))
         : Math.max(60, Math.round(colWidth * 0.55));
      const overscanY = isPanInteracting
         ? Math.max(130, Math.round(slotHeight * 1.5))
         : Math.max(92, Math.round(slotHeight * 1.15));
   ...
   const pasteFromCopyToSlot = useCallback(
      async (copy, slot) => {
         ...
         const fallbackGearbox = normalizeGearboxValue(copy.gearbox || "Manual");
         const fallbackColor =
         ...
         const optimisticId = `tmp-paste-${Date.now()}-${Math.random()
            .toString(36)
            .slice(2, 8)}`;
         ...
         dispatch(addReservationLocal(optimisticReservation));
         try {
            triggerCalendarRefresh({
               source: "shortcut",
            ...
            reservations: [
               {
                  ...
                  sector: fallbackSector,
                  gearbox:
                     fallbackGearbox.toLowerCase() === "automat"
                        ? "Automat"
                        : "Manual",
                  privateMessage: fallbackPrivateMessage,
                  color: fallbackColor,
         ...
         try {
            ...
            createdFromResponse = pickCreatedReservationFromPasteResult(
               createResult,
               {
                  startTimeToSend,
                  userIdNum,
            ...
            if (createdFromResponse?.id != null) {
               dispatch(removeReservationLocal(optimisticId));
               dispatch(addReservationLocal(createdFromResponse));
            } else {
               dispatch(
                  patchReservationLocal({
                     id: optimisticId,
            ...
            setTimeout(async () => {
               try {
                  ...
                  console.error(
                     "fetchReservationsDelta după paste eșuat a eșuat și el:",
                     err2,
                  );
               }
            }, 0);
         ...
         setTimeout(async () => {
            ...
            try {
               ...
               if (!createdFromResponse && fetched) {
                  dispatch(removeReservationLocal(optimisticId));
               }
               try {
                  triggerCalendarRefresh({
   ...
   const blockSelectedSlot = useCallback(async (slot) => {
      ...
      // optimistic: marcăm blocat instant în calendar
      try {
         triggerCalendarRefresh({
            source: "shortcut",
      ...
      try {
         ...
         console.error("Eroare la blocarea slotului (Ctrl+L):", err);
