...
function ACalendarViewOptimized({
  ...
  const DayViewWithHandlers = useMemo(() => {
    ...
    if (CustomDayViewOptimized.range) Comp.range = CustomDayViewOptimized.range;
    return Comp;
  }, [
    handleEdit,
    handleChangeColor,
      ...
      <Calendar
        ...
        formats={{
          ...
          dayRangeHeaderFormat: ({ start, end }, culture, local) => {
            ...
            if (sameMonth) {
              return `${local.format(start, "DD", culture)}–${local.format(
                end,
                "DD MMM YYYY",
                culture
              )}`;
            }
            return `${local.format(start, "DD MMM", culture)} – ${local.format(
              end,
              "DD MMM YYYY",
              culture
            )}`;
